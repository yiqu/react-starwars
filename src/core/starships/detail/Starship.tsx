...
import { useFetchStarshipQuery } from "src/core/store/sw-entities-config/swapi";
...
import { StarwarsStarships } from "src/shared/models/starwars.model";
...
function Starship() {
  ...
  const { data, isLoading, isFetching, error, isError, refetch } = useFetchStarshipQuery(starshipId!);
  ...
  const display = useMemo(() => {
    ...
    list = displayList.map((key: string) => {
      return {
        key,
        value: (`${data?.result.properties[key as keyof StarwarsStarships]}`)
      };
    });
    return list;
  }, [data]);
...
const displayList = ['model', 'starship_class', 'manufacturer', 'cost_in_credits', 'length', 'crew', 
